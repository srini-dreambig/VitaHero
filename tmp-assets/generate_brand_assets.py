#!/usr/bin/env python3
"""Generate VitaHero Play Store + launcher assets from the real app logo.
...
LOGO_PATH = ROOT / "android/app/src/main/res/drawable-nodpi/vitahero_logo.png"
...
# ───────────────────────────────────────────────────────────────
# 1. Launcher foreground (adaptive icon layer)
#    432x432 xxxhdpi. Logo centered with ~12% padding, on transparent
#    background so the system mask can reveal the matching background layer.
# ───────────────────────────────────────────────────────────────
...
def make_launcher_foreground():
    ...
    pad = 48
    target = size - 2 * pad
    # fit logo within target while preserving aspect ratio
...
def make_playstore_icon():
    ...
    target = size - 2 * pad
    logo.thumbnail((target, target), Image.Resampling.LANCZOS)
    # Use a clean white background for the store icon (the logo already has its own orange)
...
# ───────────────────────────────────────────────────────────────
# 3. Play Store feature banner (1024x500)
# ───────────────────────────────────────────────────────────────
...
def make_banner():
    ...
    # diagonal gradient from orange to coral-ish
    for y in range(h):
        t = y / h
        r = int(ORANGE[0] + (0xFF - ORANGE[0]) * t * 0.35)
        g = int(ORANGE[1] + (0x95 - ORANGE[1]) * t * 0.35)
        b = int(ORANGE[2] + (0x5F - ORANGE[2]) * t * 0.35)
    ...
    # subtle shadow/glow behind logo
    glow = Image.new("RGBA", (logo_w + 40, logo_h + 40), (0, 0, 0, 0))
    gdraw = ImageDraw.Draw(glow)
    gdraw.rounded_rectangle([0, 0, logo_w + 40, logo_h + 40], radius=24, fill=(255, 255, 255, 50))
    canvas.paste(glow, (80 - 20, h // 2 - logo_h // 2 - 20), glow)
    canvas.paste(logo, (80, h // 2 - logo_h // 2), logo)
...
# ───────────────────────────────────────────────────────────────
# Helpers for phone UI mockups
# ───────────────────────────────────────────────────────────────


def phone_canvas() -> tuple[Image.Image, ImageDraw.Draw]:
    """Return a 1080x2400 phone screenshot canvas with system status bar."""
...
def make_screenshot_growth():
    ...
    legend = [((0xF5, 0x9E, 0x0B), "P97"), ((0x1F, 0xA2, 0xDD), "P85"), ((0xF4, 0x7B, 0x20), "P50"),
              ((0x94, 0xA3, 0xB8), "P3")]
    lx = 76
