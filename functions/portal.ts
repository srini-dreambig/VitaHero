...
<style>
  ...
  *{box-sizing:border-box}
  ...
  /* ── record list ──
     ...
     is tagged, and how it is doing — each figure labelled, so nothing has to be
     read off a column header several rows above. A grid rather than a table
  ...
  /* The count of what did not fit never shrinks and is never the thing that
     gets clipped — a "+2" that is itself cut off is worse than no chip at all.
     The named chips give way first. */
  .tagset .tg.more{background:none;border-color:transparent;color:var(--ink-3);flex:none}
...
(function () {
  ...
  /**
   * A percentage as a ring, the way a gauge reads at a glance.
   *
   * Drawn rather than approximated with a coloured dot, because the shape
   * carries the number: a quarter-full ring and a three-quarter-full ring are
   * telling you different things before you have read the figure beside them.
   * A null percentage draws an empty track — "nothing to measure yet", which
  ...
  function ring(pct, tone) {
    ...
    function circle(colour, dash) {
      var n = document.createElementNS(NS, "circle");
      n.setAttribute("cx", "13"); n.setAttribute("cy", "13"); n.setAttribute("r", String(r));
      n.setAttribute("fill", "none"); n.setAttribute("stroke", colour);
      n.setAttribute("stroke-width", "3"); n.setAttribute("stroke-linecap", "round");
      if (dash !== null) {
        n.setAttribute("stroke-dasharray", dash + " " + c);
        n.setAttribute("transform", "rotate(-90 13 13)");
      }
      return n;
    }
    svg.appendChild(circle("var(--line)", null));
    if (pct !== null && pct !== undefined) {
      var v = Math.max(0, Math.min(100, Number(pct) || 0));
      svg.appendChild(circle("var(--" + (tone || "info") + ")", (c * v / 100).toFixed(2)));
    }
    return svg;
  }

  /**
   * One labelled figure in a row: an indicator, the caption, and the value.
   *
   * "mark" is whatever goes on the left — a ring, an icon, or nothing.
   */
  function metric(mark, label, value, tone) {
    return el("div", { class: "metric" },
      mark ? el("span", { class: "mi" }, mark) : null,
      el("span", { class: "mv" },
        el("span", { class: "ml" }, label),
        el("span", { class: "mn" + (tone ? " " + tone : "") }, value)));
  }

  /** The band a rate falls in. Used for the ring and the figure alike. */
  function rateTone(pct) {
    if (pct === null || pct === undefined) return "";
    return pct >= 80 ? "ok" : pct >= 50 ? "warn" : "err";
  }

  /**
   * Tag chips, with the ones that did not fit counted rather than dropped.
   * Hovering the count names them, so nothing is actually hidden.
   */
  function tagSet(tags, max) {
    ...
    return el("div", { class: "tagset" },
      ...
      rest > 0 ? el("span", { class: "tg more", title: list.join(", ") }, "+" + rest) : null);
  }

  /** A quick filter: the label, how many it would leave, and whether it is on. */
  function quickFilter(opts, current, go) {
    return el("div", { class: "qf" }, opts.map(function (o) {
      return el("button", {
        class: o[0] === current ? "on" : "",
        onclick: function () { go(o[0]); },
      }, o[2] ? icon(o[2], 13) : null, o[1],
        el("span", { class: "c" }, String(o[3])));
    }));
  }
  ...
  function freshState() {
    return {
      ...
      // here rather than on the server: the list is one row per school, which
  ...
  var SCHOOL_SORTS = [
    ["name", "Name"], ["students", "Students"], ["coverage", "Coverage"],
    ["referrals", "Open referrals"], ["onboarded", "Onboarded"],
  ];

  /**
   * The schools list.
   *
   * One row per school carrying the whole school: what it is called and where,
   * its partner code, the checks it has agreed to, and how the programme is
   * actually going there — consent, coverage, camps run, referrals still open.
   * Every figure is labelled on the row itself, so none of them has to be read
   * off a column header several rows above, and nobody has to open a school to
   * find out whether it needs attention.
   */
  function viewSchools() {
    var all = S.schools || [];
    var archived = function (x) { return x.active === false || x.status === "ARCHIVED"; };

    /**
     * Worth looking at today.
     *
     * Not "has any open referral": every school that is running has open
     * referrals, and a filter that matches everything tells you nothing. What
     * is worth surfacing is a school whose consent or coverage has stalled
     * below the point where a camp is worth holding, or whose referral queue
     * has grown past what an office can work through.
     */
    var LOW = 60, QUEUE = 10;
    function needsAttention(x) {
      if (archived(x)) return false;
      if (x.consentRate !== null && x.consentRate < LOW) return true;
      if (x.coverage !== null && x.coverage < LOW) return true;
      return (x.openReferrals || 0) >= QUEUE;
    }

    var counts = {
      all: all.length,
      active: all.filter(function (x) { return !archived(x); }).length,
      archived: all.filter(archived).length,
      attention: all.filter(needsAttention).length,
    };

    var rows = all.filter(function (x) {
      if (S.schoolFilter === "active") return !archived(x);
      if (S.schoolFilter === "archived") return archived(x);
      if (S.schoolFilter === "attention") return needsAttention(x);
      return true;
    });

    var q = (S.schoolQuery || "").trim().toLowerCase();
    if (q) {
      var digits = q.replace(/\D/g, "");
      rows = rows.filter(function (x) {
        return (x.name || "").toLowerCase().indexOf(q) >= 0
          || (x.city || "").toLowerCase().indexOf(q) >= 0
          || (x.district || "").toLowerCase().indexOf(q) >= 0
          || (x.partnerCode || "").toLowerCase().indexOf(q) >= 0
          || (x.contactName || "").toLowerCase().indexOf(q) >= 0
          || (digits && (x.contactPhone || "").replace(/\D/g, "").indexOf(digits) >= 0);
      });
    }

    var by = S.schoolSort || "name";
    rows = rows.slice().sort(function (a, b) {
      if (by === "students") return (b.studentCount || 0) - (a.studentCount || 0);
      if (by === "referrals") return (b.openReferrals || 0) - (a.openReferrals || 0);
      if (by === "onboarded") return String(b.onboardedAt || "").localeCompare(String(a.onboardedAt || ""));
      if (by === "coverage") {
        // A school with nothing to measure sorts last rather than as nought,
        // which would put every school that has not started yet at the top of
        // a list meant to show you where coverage is poor.
        var av = a.coverage === null ? -1 : a.coverage, bv = b.coverage === null ? -1 : b.coverage;
        return av - bv;
      }
    ...
    return el("div", null,
      el("div", { class: "tbar" },
        quickFilter([
          ["all", "All", null, counts.all],
          ["active", "Active", "check", counts.active],
          ["attention", "Needs attention", "alert", counts.attention],
          ["archived", "Archived", "archive", counts.archived],
        ], S.schoolFilter || "all", function (k) { set({ schoolFilter: k }); }),
        el("div", { class: "sep" }),
        el("input", { type: "text", id: "schoolq", value: S.schoolQuery || "",
          placeholder: "Name, city, code, contact or number",
          oninput: function (e) { set({ schoolQuery: e.target.value }); } }),
        el("select", { style: "width:auto", onchange: function (e) { set({ schoolSort: e.target.value }); } },
          SCHOOL_SORTS.map(function (o) {
            return el("option", { value: o[0], selected: by === o[0] }, "Sort: " + o[1]);
          })),
        el("div", { style: "flex:1" }),
        el("span", { class: "cnt" }, rows.length === all.length
      ...
      rows.length === 0
        ? el("div", { class: "card" }, el("div", { class: "empty" },
            ...
            !all.length && isOps()
              ? el("button", { class: "pri", onclick: function () { set({ view: "newSchool" }); } },
                  icon("plus", 14), " Add school")
              : null))
        : el("div", { class: "recs" }, rows.map(schoolRow)));
  }

  /** One school, whole, on one row — with the rest of it underneath on ask. */
  function schoolRow(s) {
    var off = s.active === false || s.status === "ARCHIVED";
    var open = S.schoolOpen === s.id;
    return el("div", { class: "recw" }, el("div", {
      class: "rec" + (off ? " muted" : "") + (open ? " open" : ""),
      // The row carries what you scan down a list for. The checks a school has
      ...
      el("span", { class: "rt" }, icon("school", 15)),

      el("div", { class: "rcol" },
        el("div", { class: "rid" }, s.partnerCode || "\u2014"),
        el("div", { class: "rname" }, s.name),
        el("div", { class: "rsub" },
          [s.city, s.district].filter(Boolean).join(" \u00b7 ")
          + (s.academicYear ? "  \u00b7  " + s.academicYear : ""))),

      metric(ring(s.consentRate, rateTone(s.consentRate)), "Consent",
        s.consentRate === null ? "\u2014" : s.consentRate + "%",
        s.consentRate === null ? "off" : rateTone(s.consentRate)),

      metric(ring(s.coverage, rateTone(s.coverage)), "Screened",
        s.coverage === null ? "\u2014" : s.coverage + "%",
        s.coverage === null ? "off" : rateTone(s.coverage)),

      metric(el("span", null, icon("calendar", 14)), "Camps run",
        (s.campsRun || 0) + " of " + (s.campCount || 0),
        s.campCount ? "" : "off"),

      metric(el("span", null, icon("alert", 14)), "Open referrals",
        String(s.openReferrals || 0), s.openReferrals ? "warn" : "off"),

      el("div", { class: "rcol" },
        el("span", { class: "ml" }, "On roll"),
        el("span", { class: "mn" }, String(s.studentCount || 0)),
        el("span", { class: "rsub" },
          (s.adminCount || 0) + (s.adminCount === 1 ? " member of staff" : " staff"))),

      el("div", { class: "rcol" },
        statusPill(!off && s.status === "ACTIVE" ? "ACTIVE" : s.status || "ARCHIVED")),


      el("div", { class: "row", style: "gap:2px;justify-content:flex-end",
          onclick: function (e) { e.stopPropagation(); } },
        el("button", {
          class: "ibtn expander" + (open ? " on" : ""),
          title: open ? "Hide the details" : "Everything captured at onboarding",
          "aria-expanded": open ? "true" : "false",
          onclick: function () { set({ schoolOpen: open ? "" : s.id }); },
        }, el("span", { class: "chev" + (open ? " up" : "") }, icon("chevron", 15))),
        isOps()
        ? rowMenu("sch:" + s.id, [
        ...
        : null)),
      open ? schoolDetail(s) : null);
  }

  /**
   * Everything a school told us at onboarding, under its own row.
   *
   * The row carries what you scan for — is this school on track, does it need
   * anything today. This carries what you look up: who to ring, what was
   * agreed, when it started. Putting all of it in the row meant thirteen thin
   * columns; putting it only inside the school meant opening a school to find
   * a phone number.
   */
  function schoolDetail(s) {
    function f(label, value, mono) {
      return el("div", null,
        el("span", { class: "ml" }, label),
        el("span", { class: (mono ? "mono " : "") + "dv" },
          value || el("span", { class: "muted" }, "\u2014")));
    }
    return el("div", { class: "recd" },
      el("div", { class: "dgrid" },
        f("Contact", s.contactName),
        f("Contact mobile", s.contactPhone, true),
        f("Email", s.contactEmail),
        f("Partner code", s.partnerCode, true),
        f("Academic year", s.academicYear),
        f("Camps run", cadenceLabel(s.campCadence)),
        f("Onboarded", s.onboardedAt ? fmtDate(s.onboardedAt) : ""),
        f("Children on roll", String(s.studentCount || 0)),
        f("Staff", String(s.adminCount || 0)),
  ...
  function viewCamp() {
    ...
    return el("div", null,
      // The camp's own numbers first — they belong to the camp, not to any one
      // stage — then the stage you are working in.
      el("div", { class: "stats" },
  ...
  /**
   * Oversight's screens. Partners is a judgement about a hospital; Lookups
  ...
  /**
   * A workspace's tab bar: the groups across the top, and the screens inside
   * the group you are in underneath.
   *
   * The left navigation lists destinations — Schools, Hospitals, Oversight —
   * and nothing else. What lives inside one of them belongs to that thing, not
   * to the shell around it.
   *
   * This is the third arrangement of these screens, and the first that is
   * actually an arrangement. Twelve tabs across the top of a school was a flat
   * strip nobody could hold in their head. Moving that strip into the sidebar
   * only changed where the twelve items were, and adding every other section
   * beside them made twenty-eight. Grouping is what was missing: five groups
   * of at most four, so a screen is two short decisions away and neither
   * decision has more than five options.
   *
   * groups: [key, label, [[leafKey, leafLabel, badge?] | null, ...]]
  ...
  /**
   * A school's screens, grouped by the job rather than by the screen.
   *
   * Students is the roll and how it got there. Camps is the running of them,
   * and App invites sits there because an invitation exists to get a guardian
   ...
   * A camp's stages.
   *
   * One group each, so they stay a single row in the order the day actually
  ...
  /**
   * Draw one screen, and keep the tab bar even if it cannot be drawn.
   *
   * render() already catches a throwing screen, but it catches the whole
   * workspace with it — and the tabs live inside the workspace now that they
   * are no longer in the sidebar. One screen with an unexpected response
   * would take the tabs down too, leaving no way to move to a screen that
   * works. The tabs are rendered outside this; only the content is inside.
   */
  function safely(fn) {
