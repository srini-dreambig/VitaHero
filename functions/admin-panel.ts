...
function switchTab(t){var ts=['overview','import','camps','parents','doctors','hospitals','history'];for(var i=0;i<ts.length;i++){document.getElementById('tab-'+ts[i]).classList.toggle('active',ts[i]===t);document.getElementById('content-'+ts[i]).classList.toggle('active',ts[i]===t)}var titles={overview:'Overview',import:'Import Data',camps:'Camps',parents:'Parents',doctors:'Doctors',hospitals:'Hospitals',history:'Import History'};document.getElementById('pageTitle').textContent=titles[t];if(t==='overview')loadOverview();if(t==='camps')loadCampsTab();if(t==='parents')loadParents();if(t==='doctors')loadDoctorsTab();if(t==='hospitals')loadHospitals();if(t==='history')loadHistory()}
