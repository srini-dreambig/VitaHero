...
.doc-row .form-group input{padding:8px 10px;font-size:12px}
...
<div class="app" id="mainApp">
  <aside class="sidebar" id="sidebar">
    ...
    <div class="sidebar-footer">
      <span class="ver">VitaHero Admin v1.1</span>
...
9876543210,Aarav Sharma,Rahul Sharma,M,Class 3,7,OAK2026,Oakridge International School,2026-07-15,Annual Health Camp,115,22,GOOD,GOOD,GOOD,STU001"></textarea>
    ...
    <div class="tab-content" id="content-doctors">
      <div class="card">
        ...
        <p class="helper-text">Create per-camp doctor logins. Each doctor receives an OTP to sign in to the VitaHero app and fill health checkup forms. <b>Add multiple doctors at once</b> and assign them all to the same camp.</p>
        <div class="form-group" style="margin-bottom:12px">
        ...
        <div id="doctorRowsContainer">
          <div class="doc-row" data-row="0">
            ...
            <div class="form-group"><input type="text" class="doc-specialty" placeholder="General Paediatrics" value="General Paediatrics"></div>
...
function generateDoctors(){var camp=document.getElementById('docCampSelect').value;var rows=document.querySelectorAll('#doctorRowsContainer .doc-row');var doctors=[];var errors=[];for(var i=0;i<rows.length;i++){var name=rows[i].querySelector('.doc-name').value.trim();var phone=rows[i].querySelector('.doc-phone').value.trim();var spec=rows[i].querySelector('.doc-specialty').value.trim()||'General Paediatrics';var hosp=rows[i].querySelector('.doc-hospital').value.trim();if(!name&&!phone)continue;if(!name||!phone){errors.push('Row '+(i+1)+': Missing name or phone');continue}doctors.push({doctor_name:name,phone:phone,specialty:spec,hospital:hosp})}if(doctors.length===0){document.getElementById('genDocResult').innerHTML='<div class="banner warn" style="display:flex">Please fill at least one doctor with name and phone.</div>';return}if(!camp){document.getElementById('genDocResult').innerHTML='<div class="banner warn" style="display:flex">Please select a camp first.</div>';return}if(errors.length>0&&!confirm(errors.length+' row(s) have errors and will be skipped. Continue with '+doctors.length+' valid doctor(s)?'))return;var btn=document.getElementById('genDocBtn');btn.disabled=true;btn.textContent='Generating ('+doctors.length+')...';api('POST','/api/admin/doctors/generate-batch',{doctors:doctors,school_camp_id:camp}).then(function(r){btn.disabled=false;btn.textContent='Generate Credentials';var cls=r.errors>0?'warn':'info';var msg='<div class="banner '+cls+'" style="display:flex"><span>&#10003;</span> <b>'+r.created+'</b> credential'+(r.created!==1?'s':'')+' created, <b>'+r.errors+'</b> error'+(r.errors!==1?'s':'')+'. '+r.total+' total processed.';if(r.errors>0){var errList='';for(var j=0;j<r.results.length;j++){if(r.results[j].status==='error')errList+='<br>&nbsp;&nbsp;&bull; Row '+r.results[j].row+': '+r.results[j].message}msg+=errList}msg+='</div>';if(r.created>0){var otpTable='<div style="margin-top:12px"><p style="font-size:12px;font-weight:600;color:var(--ink-soft);margin-bottom:8px">Share these OTPs with the doctors to sign in:</p><div class="table-wrap"><table><thead><tr><th>Doctor</th><th>Phone</th><th>OTP</th><th>SMS</th></tr></thead><tbody>';for(var k=0;k<r.results.length;k++){var dr=r.results[k];if(dr.status==='created'){otpTable+='<tr><td><b>'+dr.doctor_name+'</b></td><td>'+dr.phone+'</td><td><code style="font-size:14px;font-weight:700;color:var(--orange);letter-spacing:1px;background:var(--orange-soft);padding:3px 8px;border-radius:4px">'+dr.otp+'</code></td><td>'+(dr.sms_sent?'<span class="badge green">Sent</span>':'<span class="badge amber">Not sent</span>')+'</td></tr>'}}otpTable+='</tbody></table></div></div>';msg+=otpTable;resetDoctorForm();loadDoctors()}document.getElementById('genDocResult').innerHTML=msg}).catch(function(e){btn.disabled=false;btn.textContent='Generate Credentials';document.getElementById('genDocResult').innerHTML='<div class="banner warn" style="display:flex">Error: '+e.message+'</div>'})}
...
function loadDoctors(){document.getElementById('doctorsTable').innerHTML='<div class="loading"><div class="spinner"></div></div>';api('GET','/api/admin/doctors').then(function(rows){doctorsCache=rows||[];if(!rows||rows.length===0){document.getElementById('doctorsTable').innerHTML='<div class="empty-state"><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M4.8 2.3A.3.3 0 1 0 5 2H4a2 2 0 0 0-2 2v5a6 6 0 0 0 6 6v0a6 6 0 0 0 6-6V4a2 2 0 0 0-2-2h-1a.2.2 0 1 0 .2.3"/><path d="M8 15v1a6 6 0 0 0 6 6v0a6 6 0 0 0 6-6v-4"/><circle cx="20" cy="10" r="2"/></svg><p>No doctor credentials generated yet.</p></div>';return}var h='<table><thead><tr><th>Doctor</th><th>Phone</th><th>Camp</th><th>School</th><th>Camp Date</th><th>Status</th><th>Login</th><th>OTP</th><th>Action</th></tr></thead><tbody>';for(var i=0;i<rows.length;i++){var d=rows[i];var st=d.assignment_status==='ACTIVE'?'<span class="badge green">Active</span>':'<span class="badge red">Revoked</span>';var lg=d.is_logged_in?'<span class="badge blue">Logged In</span>':'<span class="badge gray">Offline</span>';var otpBtn='<button class="btn-sm" onclick="showOtpForRow('+i+')">Show OTP</button>';var rv=d.assignment_status==='ACTIVE'?'<button class="btn-sm" style="color:var(--alert);border-color:rgba(239,68,68,.3)" onclick="revokeDoctor('+d.assignment_id+')">Revoke</button>':'-';h+='<tr><td><b>'+(d.doctor_name||'')+'</b></td><td>'+(d.phone||'')+'</td><td>'+(d.camp_title||'-')+'</td><td>'+(d.school_name||'-')+'</td><td>'+(d.camp_date||'-')+'</td><td>'+st+'</td><td>'+lg+'</td><td id="docOtpCell_'+i+'">'+otpBtn+'</td><td>'+rv+'</td></tr>'}h+='</tbody></table>';document.getElementById('doctorsTable').innerHTML=h}).catch(function(e){document.getElementById('doctorsTable').innerHTML='<p style="color:var(--alert);padding:12px;font-weight:500;font-size:12px">Error: '+e.message+'</p>'})}
...
function showOtpForRow(idx){var d=doctorsCache[idx];if(!d||!d.phone)return;var cell=document.getElementById('docOtpCell_'+idx);if(!cell)return;cell.innerHTML='<div class="loading" style="padding:0"><div class="spinner" style="width:16px;height:16px;border-width:2px"></div></div>';api('POST','/api/admin/doctors/resend-otp',{phone:d.phone}).then(function(r){cell.innerHTML='<code style="font-size:13px;font-weight:700;color:var(--orange);letter-spacing:1px;background:var(--orange-soft);padding:3px 8px;border-radius:4px">'+r.otp+'</code>'}).catch(function(e){cell.innerHTML='<button class="btn-sm" onclick="showOtpForRow('+idx+')">Retry</button>';alert('Failed to get OTP: '+e.message)})}
</script>
</body>
</html>`;
}
