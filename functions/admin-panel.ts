...
9876543210,Aarav Sharma,Rahul Sharma,M,Class 3,7,OAK2026,Oakridge International School,2026-07-15,Annual Health Camp,115,22,GOOD,GOOD,GOOD,STU001"></textarea>
    ...
    <div class="tab-content" id="content-parents">
      <div class="banner warn" id="devModeBannerParents" style="display:none"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg> DEV MODE is active: invite SMS is not sent — it is only logged on the server. Set DEV_MODE=false in the worker environment to send real SMS.</div>
      <div class="card">
...
function checkDevMode(){fetch('/ping').then(function(r){return r.json()}).then(function(d){if(d.dev_mode){document.getElementById('devModeBanner').style.display='flex';var p=document.getElementById('devModeBannerParents');if(p)p.style.display='flex'}}).catch(function(){})}
...
function sendInvite(phone){if(!phone)return;phone=String(phone);openModal({title:'Send Invitation',message:'Send invitation SMS to '+phone+'?',icon:'<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"26\" height=\"26\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2\"/><circle cx=\"9\" cy=\"7\" r=\"4\"/><path d=\"M22 21v-2a4 4 0 0 0-3-3.87\"/><path d=\"M16 3.13a4 4 0 0 1 0 7.75\"/></svg>',iconBg:'rgba(244,123,32,.12)',iconColor:'var(--orange)',confirmText:'Send',cancelText:'Cancel',onClose:function(ok){if(!ok)return;openModal({title:'Sending...',message:'Please wait while the invitation is sent.',confirmText:false,cancelText:false,icon:'<div class=\"spinner\"></div>'});api('POST','/api/admin/invite',{phones:[phone],force:true}).then(function(d){
  var allOk=d.invited>0 && (!d.details || d.details.every(function(x){return x.status==='sent'||x.status==='dev_bypass'}));
  var devBypass=d.devMode || (d.details && d.details.some(function(x){return x.status==='dev_bypass'}));
  var failedDetails=d.details ? d.details.filter(function(x){return x.status==='failed' || x.status==='skipped'}) : [];
  var msg='Invitation result: '+d.invited+' sent, '+d.skipped.length+' skipped.';
  if(devBypass)msg+='\n\nSMS was bypassed because DEV MODE is active. The parent will not receive the invite text. Set DEV_MODE=false in the worker environment to send real SMS.';
  if(failedDetails.length>0){
    msg+='\n\nDetails:\n'+failedDetails.map(function(x){return x.phone+' — '+x.status+(x.reason?': '+x.reason:'')}).join('\n');
  }
  openModal({title: devBypass ? 'Invitation (Dev Mode)' : (allOk ? 'Invitation Sent' : 'Invitation Partial'),message:msg,confirmText:'OK',cancelText:false,iconBg: devBypass ? 'rgba(245,158,11,.13)' : 'rgba(16,185,129,.12)',iconColor: devBypass ? '#B45309' : 'var(--good)',icon: devBypass ? '<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"26\" height=\"26\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z\"/><line x1=\"12\" y1=\"9\" x2=\"12\" y2=\"13\"/><line x1=\"12\" y1=\"17\" x2=\"12.01\" y2=\"17\"/></svg>' : '<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"26\" height=\"26\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2.5\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M20 6 9 17l-5-5\"/></svg>',onClose:function(){loadParents()}})
}).catch(function(e){openModal({title:'Invite Failed',message:'Failed to send invite: '+e.message,confirmText:'OK',cancelText:false,iconBg:'rgba(239,68,68,.12)',iconColor:'var(--alert)',icon:'<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"26\" height=\"26\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><circle cx=\"12\" cy=\"12\" r=\"10\"/><path d=\"M15 9l-6 6\"/><path d=\"m9 9 6 6\"/></svg>'})})}})}
