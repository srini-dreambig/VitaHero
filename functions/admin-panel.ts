...
9876543210,Aarav Sharma,Rahul Sharma,M,Class 3,7,OAK2026,Oakridge International School,2026-07-15,Annual Health Camp,115,22,GOOD,GOOD,GOOD,STU001"></textarea>
    ...
    <div class="tab-content" id="content-students">
      <div class="card">
        ...
        <p class="helper-text">Add one parent and child manually &mdash; runs the exact same pipeline as CSV import: creates the parent, the student record, and the invite link. To also register the child into a camp, fill both Camp Title and Camp Date.</p>
        <form id="apsForm" onsubmit="event.preventDefault();addParentStudent()">
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;margin-bottom:12px">
            ...
            <div class="form-group" style="margin:0"><label>School *</label><input type="text" id="apsSchool" placeholder="e.g. Kallam Public School"></div>
            ...
            <div class="form-group" style="margin:0"><label>Camp Title</label><input type="text" id="apsCampTitle" placeholder="e.g. Annual Health Camp"></div>
            <div class="form-group" style="margin:0"><label>Camp Date</label><input type="date" id="apsCampDate"></div>
          </div>
          <div style="display:flex;gap:10px;align-items:center">
...
function switchTab(t){var ts=['overview','import','camps','parents','students','doctors','hospitals','history'];for(var i=0;i<ts.length;i++){document.getElementById('tab-'+ts[i]).classList.toggle('active',ts[i]===t);document.getElementById('content-'+ts[i]).classList.toggle('active',ts[i]===t)}var titles={overview:'Overview',import:'Import Data',camps:'Camps',parents:'Parents',students:'Kids / Students',doctors:'Doctors',hospitals:'Hospitals',history:'Import History'};document.getElementById('pageTitle').textContent=titles[t];if(t==='overview')loadOverview();if(t==='camps')loadCampsTab();if(t==='parents')loadParents();if(t==='students')loadStudents();if(t==='doctors')loadDoctorsTab();if(t==='hospitals')loadHospitals();if(t==='history')loadHistory()}
...
function loadStudents(){var s=document.getElementById('studentSearch').value.trim();var qs=s?('?q='+encodeURIComponent(s)):'';document.getElementById('studentsTable').innerHTML='<div class="loading"><div class="spinner"></div></div>';api('GET','/api/admin/students'+qs).then(function(rows){if(!rows||rows.length===0){document.getElementById('studentsTable').innerHTML='<div class="empty-state"><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21.42 10.922a1 1 0 0 0-.019-1.838L12.83 5.18a2 2 0 0 0-1.66 0L2.6 9.08a1 1 0 0 0 0 1.832l8.57 3.908a2 2 0 0 0 1.66 0z"/><path d="M22 10v6"/><path d="M6 12.5V16a6 3 0 0 0 12 0v-3.5"/></svg><p>No students found. Imported students appear here with their parent\\'s details.</p></div>';return}var h='<table><thead><tr><th>Student</th><th>Age</th><th>Gender</th><th>Grade</th><th>Height / Weight</th><th>School</th><th>Camps</th><th>Parent</th><th>Parent Phone</th><th>Dental / Eye / Nutrition</th><th>Last Checkup</th><th>Parent Login</th></tr></thead><tbody>';for(var i=0;i<rows.length;i++){var k=rows[i];var login=k.parent_logged_in?'<span class="badge green">Active</span>':'<span class="badge amber">Pending</span>';var hw=(k.height_cm||0)+' cm / '+(k.weight_kg||0)+' kg';var health=[k.dental||'-',k.eyesight||'-',k.nutrition||'-'].join(' / ');var campHtml=(k.camps||[]).length?k.camps.map(function(c){return '<b>'+(c.title||'Camp')+'</b>'+(c.date?' <span style="font-size:11px;color:var(--ink-faint)">'+c.date+'</span>':'')}).join('<br>'):'-';h+='<tr><td><b>'+(k.name||'-')+'</b>'+(k.student_ref?'<br><span style="font-size:11px;color:var(--ink-faint)">'+k.student_ref+'</span>':'')+'</td><td>'+(k.age!=null?k.age:'-')+'</td><td>'+(k.gender||'-')+'</td><td>'+(k.grade||'-')+'</td><td>'+hw+'</td><td>'+(k.school||'-')+'</td><td>'+campHtml+'</td><td>'+(k.parent_name||'-')+'</td><td>'+(k.parent_phone||'-')+'</td><td>'+health+'</td><td>'+(k.last_checkup||'Not yet')+'</td><td>'+login+'</td></tr>'}h+='</tbody></table>';document.getElementById('studentsTable').innerHTML=h}).catch(function(e){document.getElementById('studentsTable').innerHTML='<p style="color:var(--alert);padding:12px;font-weight:500;font-size:12px">Error: '+e.message+'</p>'})}
function addParentStudent(){var parentName=document.getElementById('apsParentName').value.trim();var phone=document.getElementById('apsPhone').value.trim();var studentName=document.getElementById('apsStudentName').value.trim();var schoolName=document.getElementById('apsSchool').value.trim();var age=parseInt(document.getElementById('apsAge').value,10)||0;var gender=document.getElementById('apsGender').value;var grade=document.getElementById('apsGrade').value.trim();var studentId=document.getElementById('apsStudentId').value.trim();var campTitle=document.getElementById('apsCampTitle').value.trim();var campDate=document.getElementById('apsCampDate').value;var heightCm=parseFloat(document.getElementById('apsHeight').value)||0;var weightKg=parseFloat(document.getElementById('apsWeight').value)||0;if(!parentName||!phone||!studentName||!schoolName){document.getElementById('apsResult').innerHTML='<div class="banner warn" style="display:flex">Please fill Parent Name, Parent Phone, Student Name and School.</div>';return}
var row={phone:phone,parentName:parentName,studentName:studentName,schoolName:schoolName,dental:'GOOD',eyesight:'GOOD',nutrition:'GOOD'};if(age>0)row.age=age;if(gender)row.gender=gender;if(grade)row.grade=grade;if(studentId)row.studentId=studentId;if(campTitle)row.campTitle=campTitle;if(campDate)row.campDate=campDate;if(heightCm>0)row.heightCm=heightCm;if(weightKg>0)row.weightKg=weightKg;
...
document.getElementById('apsResult').innerHTML=h;if(ok)document.getElementById('apsForm').reset();loadStudents();loadOverview()}).catch(function(e){btn.disabled=false;btn.textContent='Add Parent & Student';document.getElementById('apsResult').innerHTML='<div class="banner warn" style="display:flex">Error: '+e.message+'</div>'})}
