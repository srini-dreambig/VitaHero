    ...
    <div class="tab-content" id="content-hospitals">
      <div class="card">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:12px">
          <div class="form-group" style="margin:0">
            <label>Latitude <span style="color:var(--ink-faint);font-weight:400">(for distance sort)</span></label>
            <input type="text" id="hospLat" placeholder="17.4156">
          </div>
          <div class="form-group" style="margin:0">
            <label>Longitude</label>
            <input type="text" id="hospLng" placeholder="78.4347">
          </div>
          <div class="form-group" style="margin:0">
            <label>Rating</label>
            <input type="text" id="hospRating" placeholder="4.5" value="4.5">
          </div>
        </div>
...
function doImport(){var csv=document.getElementById('csvInput').value.trim();if(!csv){alert('Please browse a CSV file or paste CSV data first.');return}var rows=parseCSV(csv);if(rows.length===0){alert('No data rows found in CSV.');return}if(rows.length>2000){alert('Too many rows (max 2000). Split the file.');return}var dry=document.getElementById('dryRun').checked;var gen=document.getElementById('generateLinks').checked;if(!dry&&!confirm('Import '+rows.length+' rows'+(gen?' and generate invite links':'')+'?'))return;var btn=document.getElementById('importBtn');btn.disabled=true;btn.textContent=dry?'Processing (dry run)...':'Importing...';api('POST','/api/admin/import',{rows:rows,dryRun:dry,generateLinks:dry?false:gen,filename:'admin_panel_'+new Date().toISOString().slice(0,10)}).then(function(r){btn.disabled=false;btn.textContent='Import Data';showImportResults(r);if(!dry)loadOverview()}).catch(function(e){btn.disabled=false;btn.textContent='Import Data';alert('Import failed: '+e.message)})}
var lastImportLinks=[];
function showImportResults(r){var el=document.getElementById('importResults');el.style.display='block';var linked=r.linked||0;var s='<div style="margin-bottom:10px;display:flex;align-items:center;gap:8px;flex-wrap:wrap;font-size:12px"><span class="badge '+(r.dryRun?'amber':'green')+'">'+(r.dryRun?'DRY RUN':'IMPORTED')+'</span> <b>'+r.total+'</b> rows: <span style="color:var(--good);font-weight:600">'+r.created+' created</span>, <span style="color:var(--blue);font-weight:600">'+r.updated+' updated</span>, <span style="color:var(--alert);font-weight:600">'+r.errors+' errors</span>';if(linked>0)s+=', <span style="color:var(--orange);font-weight:600">'+linked+' links generated</span>';s+='</div>';document.getElementById('importSummary').innerHTML=s;lastImportLinks=[];var h='<table><thead><tr><th>Row</th><th>Phone</th><th>Student</th><th>Status</th><th>SMS</th><th>Invite Link</th><th>Message</th></tr></thead><tbody>';for(var i=0;i<r.results.length;i++){var row=r.results[i];var bg=row.status==='created'?'green':row.status==='updated'?'blue':row.status==='skipped'?'amber':'red';var linkCell='-';if(row.link){lastImportLinks.push({phone:row.phone,link:row.link});linkCell='<button class="btn-sm" onclick="copyText(&#39;'+row.link+'&#39;,this)">Copy Link</button>'}var smsCell=row.smsSent?'<span class="badge green">Sent</span>':(row.link?'<span class="badge amber">Not sent</span>':'-');h+='<tr><td>'+row.row+'</td><td>'+(row.phone||'')+'</td><td>'+(row.student||'')+'</td><td><span class="badge '+bg+'">'+row.status+'</span></td><td>'+smsCell+'</td><td>'+linkCell+'</td><td>'+(row.message||'')+'</td></tr>'}h+='</tbody></table>';document.getElementById('importTableWrap').innerHTML=h;var copyAllBtn=document.getElementById('copyAllLinksBtn');if(copyAllBtn)copyAllBtn.style.display=lastImportLinks.length>0?'inline-flex':'none'}
...
function loadHospitals(){document.getElementById('hospitalsTable').innerHTML='<div class="loading"><div class="spinner"></div></div>';api('GET','/api/admin/hospitals').then(function(rows){hospitalsCache=rows||[];var selects=document.querySelectorAll('#doctorRowsContainer .doc-hospital');for(var i=0;i<selects.length;i++){var cur=selects[i].value;selects[i].innerHTML=hospitalOptionsHtml(cur)}if(!rows||rows.length===0){document.getElementById('hospitalsTable').innerHTML='<div class="empty-state"><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 6v4"/><path d="M14 14h-4"/><path d="M14 18h-4"/><path d="M14 8h-4"/><path d="M18 12h2a2 2 0 0 1 2 2v6a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2v-6a2 2 0 0 1 2-2h2"/><path d="M6 12V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v8"/></svg><p>No hospitals added yet. Add one above so parents can find where your doctors practice.</p></div>';return}var h='<table><thead><tr><th>Name</th><th>Address</th><th>City / Area</th><th>Rating</th><th>Camp Partner</th><th>Actions</th></tr></thead><tbody>';for(var i=0;i<rows.length;i++){var hh=rows[i];var partner=hh.is_camp_partner?'<span class="badge orange">Yes</span>':'<span class="badge gray">No</span>';h+='<tr><td><b>'+(hh.name||'')+'</b></td><td>'+(hh.address||'<span style="color:var(--alert)">Missing</span>')+'</td><td>'+(hh.city||'-')+(hh.district?' / '+hh.district:'')+'</td><td>'+(hh.rating||'-')+'</td><td>'+partner+'</td><td><button class="btn-sm" onclick="editHospital('+i+')">Edit</button></td></tr>'}h+='</tbody></table>';document.getElementById('hospitalsTable').innerHTML=h}).catch(function(e){document.getElementById('hospitalsTable').innerHTML='<p style="color:var(--alert);padding:12px;font-weight:500;font-size:12px">Error: '+e.message+'</p>'})}
function saveHospital(){var name=document.getElementById('hospName').value.trim();var address=document.getElementById('hospAddress').value.trim();var city=document.getElementById('hospCity').value.trim();var district=document.getElementById('hospDistrict').value.trim();var phone=document.getElementById('hospPhone').value.trim();var lat=document.getElementById('hospLat').value.trim();var lng=document.getElementById('hospLng').value.trim();var rating=document.getElementById('hospRating').value.trim();var campPartner=document.getElementById('hospCampPartner').checked;if(!name||!address||!city){document.getElementById('hospResult').innerHTML='<div class="banner warn" style="display:flex">Please fill Hospital Name, Address, and City.</div>';return}var btn=document.getElementById('hospSaveBtn');var editId=btn.getAttribute('data-edit-id');btn.disabled=true;btn.textContent=editId?'Updating...':'Saving...';var payload={name:name,address:address,city:city,district:district,phone:phone,lat:lat,lng:lng,rating:rating,is_camp_partner:campPartner};var method=editId?'PUT':'POST';var url=editId?('/api/admin/hospitals/'+editId):'/api/admin/hospitals';api(method,url,payload).then(function(r){btn.disabled=false;btn.textContent='Save Hospital';btn.removeAttribute('data-edit-id');document.getElementById('hospCancelBtn').style.display='none';document.getElementById('hospFormHint').textContent='Add the hospitals your doctors work at, with a full address, so parents can find the nearest one to book an appointment. Fields marked with * are required.';document.getElementById('hospResult').innerHTML='<div class="banner info" style="display:flex"><span>&#10003;</span> Hospital '+(editId?'updated':'added')+': <b>'+(r.name||name)+'</b></div>';resetHospitalForm();loadHospitals()}).catch(function(e){btn.disabled=false;btn.textContent=editId?'Update Hospital':'Save Hospital';document.getElementById('hospResult').innerHTML='<div class="banner warn" style="display:flex">Failed: '+e.message+'</div>'})}
function editHospital(idx){var hh=hospitalsCache[idx];if(!hh)return;document.getElementById('hospName').value=hh.name||'';document.getElementById('hospAddress').value=hh.address||'';document.getElementById('hospCity').value=hh.city||'';document.getElementById('hospDistrict').value=hh.district||'';document.getElementById('hospPhone').value=hh.phone||'';document.getElementById('hospLat').value=hh.lat!=null?hh.lat:'';document.getElementById('hospLng').value=hh.lng!=null?hh.lng:'';document.getElementById('hospRating').value=hh.rating||4.5;document.getElementById('hospCampPartner').checked=!!hh.is_camp_partner;var btn=document.getElementById('hospSaveBtn');btn.textContent='Update Hospital';btn.setAttribute('data-edit-id',hh.id);document.getElementById('hospCancelBtn').style.display='inline-flex';document.getElementById('hospFormHint').textContent='Editing hospital: '+(hh.name||'')+'. Make changes and click Update Hospital.';document.getElementById('hospResult').innerHTML='';window.scrollTo({top:0,behavior:'smooth'})}
...
function resetHospitalForm(){document.getElementById('hospName').value='';document.getElementById('hospAddress').value='';document.getElementById('hospCity').value='';document.getElementById('hospDistrict').value='';document.getElementById('hospPhone').value='';document.getElementById('hospLat').value='';document.getElementById('hospLng').value='';document.getElementById('hospRating').value='4.5';document.getElementById('hospCampPartner').checked=false}
