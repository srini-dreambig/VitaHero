...
.sidebar nav .nav-group{margin:12px 8px 4px;font-size:9px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;color:rgba(255,255,255,.35);white-space:nowrap}
.sidebar nav .nav-group:first-child{margin-top:4px}
.sidebar.collapsed nav .nav-group{visibility:hidden;margin:6px 0;height:6px}
...
th{text-align:left;padding:9px 12px;font-size:10px;color:var(--ink-soft);text-transform:uppercase;letter-spacing:.3px;border-bottom:2px solid var(--hair);font-weight:600;background:var(--muted);white-space:nowrap}
...
<div class="app" id="mainApp">
  <aside class="sidebar" id="sidebar">
    ...
    <nav>
      ...
      <div class="nav-group">Step 1 &middot; Camps &amp; Data</div>
      ...
      <div class="nav-group">Step 2 &middot; People</div>
      <a id="tab-students" onclick="switchTab('students')">
        ...
        <span class="label">Kids / Students</span>
      </a>
      ...
      <div class="nav-group">Care Network</div>
      <a id="tab-hospitals" onclick="switchTab('hospitals')">
        <span class="ico"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 6v4"/><path d="M14 14h-4"/><path d="M14 18h-4"/><path d="M14 8h-4"/><path d="M18 12h2a2 2 0 0 1 2 2v6a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2v-6a2 2 0 0 1 2-2h2"/><path d="M6 12V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v8"/></svg></span>
    ...
    <div class="tab-content active" id="content-overview">
      <div class="stat-grid">
        ...
        <div class="stat-card amber">
          ...
          <div class="info"><div class="label">Invite Links Generated</div><div class="value" id="stat-invites">--</div></div>
        </div>
      </div>
      <div class="card">
        ...
        <ol class="flow-list">
          ...
          <li><b>Invite sent automatically</b>: Each provisioned parent is texted their unique invite link via SMS the moment you click Invite (Copy/Share buttons remain as a manual backup).</li>
          <li><b>Parent downloads app</b>: The link opens a landing page with a Google Play Store button.</li>
          <li><b>Parent logs in</b>: Parent enters their registered phone number. Only provisioned numbers can sign in.</li>
          <li><b>OTP verification</b>: Firebase sends a one-time code directly to the parent's phone and they access their child's health data.</li>
        </ol>
      </div>
    </div>
...
9876543210,Aarav Sharma,Rahul Sharma,M,Class 3,7,OAK2026,Oakridge International School,2026-07-15,Annual Health Camp,115,22,GOOD,GOOD,GOOD,STU001"></textarea>
        ...
        <div class="checkbox-row"><input type="checkbox" id="resendExisting"><label for="resendExisting">Also re-text parents who were already invited before (off by default — only new/never-invited parents get an SMS)</label></div>
      ...
      <div class="card" id="importResults" style="display:none">
        <div class="card-header">
          <h3 style="font-size:14px;font-weight:600">Import Results</h3>
          <button class="btn-sm" id="copyAllLinksBtn" onclick="copyAllLinks()" style="display:none">Copy All Links</button>
        </div>
    ...
    <div class="tab-content" id="content-camps">
      <div class="card" id="campFormCard" style="display:none">
        <div class="card-header">
      ...
      <div class="card">
        <div class="card-header">
          ...
          <button class="btn-sm" onclick="toggleForm('campFormCard',this,'Add Camp')">Add Camp</button>
    ...
    <div class="tab-content" id="content-parents">
      <div class="card">
        ...
        <div class="search-row">
          ...
          <button class="btn-sm" onclick="switchTab('students');setTimeout(function(){var c=document.getElementById('apsFormCard');if(c)c.style.display='';var el=document.getElementById('apsParentName');if(el)el.focus()},150)">+ Add Parent / Student</button>
        </div>
    ...
    <div class="tab-content" id="content-students">
      <div class="card" id="apsFormCard" style="display:none">
        <div class="card-header">
          <div class="card-title"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> Add Parent &amp; Student</div>
        </div>
        <p class="helper-text">Add one parent and child manually &mdash; runs the exact same pipeline as CSV import: creates the parent, the student record, and the invite link. Pick the School from the dropdown; optionally pick a Camp to register the child into that exact camp.</p>
        <form id="apsForm" onsubmit="event.preventDefault();addParentStudent()">
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;margin-bottom:12px">
            <div class="form-group" style="margin:0"><label>Parent Name *</label><input type="text" id="apsParentName" placeholder="e.g. Ramesh Kumar"></div>
            <div class="form-group" style="margin:0"><label>Parent Phone *</label><input type="tel" id="apsPhone" placeholder="10-digit mobile number"></div>
            <div class="form-group" style="margin:0"><label>Student Name *</label><input type="text" id="apsStudentName" placeholder="e.g. Arjun Krishna"></div>
            <div class="form-group" style="margin:0"><label>School *</label><select id="apsSchoolSelect"><option value="">Loading schools...</option></select></div>
            <div class="form-group" style="margin:0"><label>Age</label><input type="number" id="apsAge" min="2" max="21"></div>
            <div class="form-group" style="margin:0"><label>Gender</label><select id="apsGender"><option value=""></option><option value="Male">Male</option><option value="Female">Female</option><option value="Other">Other</option></select></div>
            <div class="form-group" style="margin:0"><label>Grade / Class</label><input type="text" id="apsGrade" placeholder="e.g. Class 5"></div>
            <div class="form-group" style="margin:0"><label>Student ID</label><input type="text" id="apsStudentId" placeholder="Roll / admission number"></div>
            <div class="form-group" style="margin:0"><label>Height (cm)</label><input type="number" id="apsHeight" step="0.1" min="0"></div>
            <div class="form-group" style="margin:0"><label>Weight (kg)</label><input type="number" id="apsWeight" step="0.1" min="0"></div>
            <div class="form-group" style="margin:0"><label>Camp (optional)</label><select id="apsCampSelect"><option value="">No camp registration</option></select></div>
          </div>
          <div style="display:flex;gap:10px;align-items:center">
            <button type="submit" class="btn-primary" id="apsSaveBtn">Add Parent &amp; Student</button>
            <button type="button" class="btn-secondary" onclick="document.getElementById(&#39;apsForm&#39;).reset();document.getElementById(&#39;apsResult&#39;).innerHTML=&#39;&#39;">Clear</button>
          </div>
          <div id="apsResult" style="margin-top:12px"></div>
        </form>
      </div>
      <div class="card">
        <div class="card-header"><div class="card-title"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21.42 10.922a1 1 0 0 0-.019-1.838L12.83 5.18a2 2 0 0 0-1.66 0L2.6 9.08a1 1 0 0 0 0 1.832l8.57 3.908a2 2 0 0 0 1.66 0z"/><path d="M22 10v6"/><path d="M6 12.5V16a6 3 0 0 0 12 0v-3.5"/></svg> Kids / Students</div><button class="btn-sm" onclick="toggleForm('apsFormCard',this,'Add Parent / Student')">Add Parent / Student</button></div>
        <div class="search-row">
          <input type="text" id="studentSearch" placeholder="Search by student, parent, phone or school..." onkeydown="if(event.key==='Enter')loadStudents()">
          <button class="btn-sm" onclick="loadStudents()"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg> Search</button>
          <button class="btn-sm" onclick="loadStudents()"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M8 16H3v5"/></svg> Refresh</button>
        </div>
        <div class="table-wrap"><div id="studentsTable"><div class="empty-state"><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21.42 10.922a1 1 0 0 0-.019-1.838L12.83 5.18a2 2 0 0 0-1.66 0L2.6 9.08a1 1 0 0 0 0 1.832l8.57 3.908a2 2 0 0 0 1.66 0z"/><path d="M22 10v6"/><path d="M6 12.5V16a6 3 0 0 0 12 0v-3.5"/></svg><p>Click Search to load students imported from your data files</p></div></div></div>
      </div>
    </div>
    ...
    <div class="tab-content" id="content-hospitals">
      <div class="card" id="hospFormCard" style="display:none">
        <div class="card-header">
          <div class="card-title"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 6v4"/><path d="M14 14h-4"/><path d="M14 18h-4"/><path d="M14 8h-4"/><path d="M18 12h2a2 2 0 0 1 2 2v6a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2v-6a2 2 0 0 1 2-2h2"/><path d="M6 12V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v8"/></svg> Add / Edit Hospital</div>
        </div>
        <p class="helper-text" id="hospFormHint">Add the hospitals your doctors work at, with a full address, so parents can find the nearest one to book an appointment. Fields marked with * are required.</p>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
          <div class="form-group" style="margin:0">
            <label>Hospital / Clinic Name *</label>
            <input type="text" id="hospName" placeholder="Rainbow Children's Hospital">
          </div>
          <div class="form-group" style="margin:0">
            <label>Phone</label>
            <input type="text" id="hospPhone" placeholder="+9140xxxxxxx">
          </div>
        </div>
        <div class="form-group" style="margin-bottom:12px">
          <label>Full Address *</label>
          <input type="text" id="hospAddress" placeholder="Road No 2, Banjara Hills, Hyderabad">
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
          <div class="form-group" style="margin:0">
            <label>City *</label>
            <input type="text" id="hospCity" placeholder="Hyderabad">
          </div>
          <div class="form-group" style="margin:0">
            <label>District / Area</label>
            <input type="text" id="hospDistrict" placeholder="Banjara Hills">
          </div>
        </div>
        <div class="form-group" style="margin-bottom:12px">
          <label>Pincode <span style="color:var(--ink-faint);font-weight:400">(helps parents find the nearest hospital)</span></label>
          <input type="text" id="hospPincode" placeholder="500034" maxlength="6">
        </div>
        <div class="checkbox-row"><input type="checkbox" id="hospCampPartner"><label for="hospCampPartner">Camp partner hospital (shown first to parents)</label></div>
        <div style="display:flex;gap:10px;align-items:center;margin-top:4px">
          <button class="btn-primary" id="hospSaveBtn" onclick="saveHospital()">Save Hospital</button>
          <button class="btn-secondary" id="hospCancelBtn" onclick="cancelHospitalEdit()" style="display:none">Cancel</button>
        </div>
        <div id="hospResult" style="margin-top:10px"></div>
      </div>
      <div class="card">
        <div class="card-header">
          <div class="card-title">All Hospitals</div>
          <button class="btn-sm" onclick="toggleForm('hospFormCard',this,'Add Hospital')">Add Hospital</button>
          <button class="btn-sm" onclick="loadHospitals()"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M8 16H3v5"/></svg> Refresh</button>
        </div>
        <div class="table-wrap"><div id="hospitalsTable"><div class="empty-state"><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 6v4"/><path d="M14 14h-4"/><path d="M14 18h-4"/><path d="M14 8h-4"/><path d="M18 12h2a2 2 0 0 1 2 2v6a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2v-6a2 2 0 0 1 2-2h2"/><path d="M6 12V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v8"/></svg><p>Click Refresh to load hospitals</p></div></div></div>
      </div>
    </div>

    <div class="tab-content" id="content-doctors">
      <div class="card" id="docFormCard" style="display:none">
        <div class="card-header">
        ...
        <div id="doctorRowsContainer">
          <div class="doc-row" data-row="0">
            ...
            <div class="form-group"><select class="doc-hospital"><option value="">No hospital linked yet</option></select></div>
        ...
        <p class="helper-text" style="margin-top:8px">Don't see the hospital? Add it first in the <b>Hospitals</b> tab with its full address, then come back here to assign doctors to it — this is how parents find the nearest hospital to book with your doctors.</p>
        <div style="display:flex;gap:10px;align-items:center;margin-top:4px">
...
var docRowCounter=0;
var campsCache=[];var doctorsCache=[];var hospitalsCache=[];
...
function switchTab(t){var ts=['overview','import','camps','parents','students','doctors','hospitals','history'];for(var i=0;i<ts.length;i++){document.getElementById('tab-'+ts[i]).classList.toggle('active',ts[i]===t);document.getElementById('content-'+ts[i]).classList.toggle('active',ts[i]===t)}var titles={overview:'Overview',import:'Import Data',camps:'Camps',parents:'Parents',students:'Kids / Students',doctors:'Doctors',hospitals:'Hospitals',history:'Import History'};document.getElementById('pageTitle').textContent=titles[t];if(t==='overview')loadOverview();if(t==='camps')loadCampsTab();if(t==='parents')loadParents();if(t==='students'){loadStudents();loadApsOptions()}if(t==='doctors')loadDoctorsTab();if(t==='hospitals')loadHospitals();if(t==='history')loadHistory()}
...
function doImport(){var csv=document.getElementById('csvInput').value.trim();if(!csv){alert('Please browse a CSV file or paste CSV data first.');return}var rows=parseCSV(csv);if(rows.length===0){alert('No data rows found in CSV.');return}if(rows.length>2000){alert('Too many rows (max 2000). Split the file.');return}var dry=document.getElementById('dryRun').checked;var gen=document.getElementById('generateLinks').checked;var resend=document.getElementById('resendExisting').checked;if(!dry&&!confirm('Import '+rows.length+' rows'+(gen?' and generate invite links':'')+(resend?' (re-texting already-invited parents too)':'')+'?'))return;var btn=document.getElementById('importBtn');btn.disabled=true;btn.textContent=dry?'Processing (dry run)...':'Importing...';api('POST','/api/admin/import',{rows:rows,dryRun:dry,generateLinks:dry?false:gen,resendExisting:dry?false:resend,filename:'admin_panel_'+new Date().toISOString().slice(0,10)}).then(function(r){btn.disabled=false;btn.textContent='Import Data';showImportResults(r);if(!dry)loadOverview()}).catch(function(e){btn.disabled=false;btn.textContent='Import Data';alert('Import failed: '+e.message)})}
var lastImportLinks=[];
function showImportResults(r){var el=document.getElementById('importResults');el.style.display='block';var linked=r.linked||0;var s='<div style="margin-bottom:10px;display:flex;align-items:center;gap:8px;flex-wrap:wrap;font-size:12px"><span class="badge '+(r.dryRun?'amber':'green')+'">'+(r.dryRun?'DRY RUN':'IMPORTED')+'</span> <b>'+r.total+'</b> rows: <span style="color:var(--good);font-weight:600">'+r.created+' created</span>, <span style="color:var(--blue);font-weight:600">'+r.updated+' updated</span>, <span style="color:var(--alert);font-weight:600">'+r.errors+' errors</span>';if(linked>0)s+=', <span style="color:var(--orange);font-weight:600">'+linked+' links generated</span>';s+='</div>';document.getElementById('importSummary').innerHTML=s;lastImportLinks=[];var h='<table><thead><tr><th>Row</th><th>Phone</th><th>Student</th><th>Status</th><th>SMS</th><th>Invite Link</th><th>Message</th></tr></thead><tbody>';for(var i=0;i<r.results.length;i++){var row=r.results[i];var bg=row.status==='created'?'green':row.status==='updated'?'blue':row.status==='skipped'?'amber':'red';var linkCell='-';if(row.link){lastImportLinks.push({phone:row.phone,link:row.link});linkCell='<button class="btn-sm" onclick="copyText(&#39;'+row.link+'&#39;,this)">Copy Link</button>'}var smsCell;if(row.alreadyInvited&&!row.smsSent){smsCell='<span class="badge gray">Already invited</span> <button class="btn-sm" onclick="resendInvite(&#39;'+(row.phone||'')+'&#39;,this)">Resend</button>'}else if(row.smsSent){smsCell='<span class="badge green">Sent</span>'}else if(row.link){smsCell='<span class="badge amber">Not sent</span>'}else{smsCell='-'}h+='<tr><td>'+row.row+'</td><td>'+(row.phone||'')+'</td><td>'+(row.student||'')+'</td><td><span class="badge '+bg+'">'+row.status+'</span></td><td>'+smsCell+'</td><td>'+linkCell+'</td><td>'+(row.message||'')+'</td></tr>'}h+='</tbody></table>';document.getElementById('importTableWrap').innerHTML=h;var copyAllBtn=document.getElementById('copyAllLinksBtn');if(copyAllBtn)copyAllBtn.style.display=lastImportLinks.length>0?'inline-flex':'none'}
function resendInvite(phone,btn){if(!phone)return;if(btn){btn.disabled=true;btn.textContent='Sending...'}api('POST','/api/admin/invite',{phones:[phone]}).then(function(d){var detail=d.details&&d.details[0];if(btn){btn.disabled=false;if(detail&&detail.smsSent){btn.textContent='Resent!';var td=btn.parentNode;td.innerHTML='<span class="badge green">Sent</span>'}else{btn.textContent='Resend';alert(detail&&detail.smsReason?detail.smsReason:'Could not resend invite.')}}}).catch(function(e){if(btn){btn.disabled=false;btn.textContent='Resend'}alert('Resend failed: '+e.message)})}
function copyText(text,btn){navigator.clipboard.writeText(text).then(function(){if(btn){var orig=btn.textContent;btn.textContent='Copied!';setTimeout(function(){btn.textContent=orig},1500)}}).catch(function(){prompt('Copy this link:',text)})}
function copyAllLinks(){if(lastImportLinks.length===0)return;var text=lastImportLinks.map(function(x){return x.phone+' - '+x.link}).join('\\n');navigator.clipboard.writeText(text).then(function(){var btn=document.getElementById('copyAllLinksBtn');if(btn){var orig=btn.textContent;btn.textContent='Copied '+lastImportLinks.length+' links!';setTimeout(function(){btn.textContent=orig},1800)}}).catch(function(){prompt('Copy these links:',text)})}
function loadParents(){var s=document.getElementById('parentSearch').value.trim();var qs=s?('?q='+encodeURIComponent(s)):'';document.getElementById('parentsTable').innerHTML='<div class="loading"><div class="spinner"></div></div>';api('GET','/api/admin/parents'+qs).then(function(rows){if(!rows||rows.length===0){document.getElementById('parentsTable').innerHTML='<div class="empty-state"><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg><p>No provisioned parents found.</p></div>';return}var h='<table><thead><tr><th>Phone</th><th>Parent Name</th><th>School</th><th>Kids</th><th>Link Generated</th><th>Status</th><th>Action</th></tr></thead><tbody>';for(var i=0;i<rows.length;i++){var p=rows[i];var inv=p.invited_at?'<span class="badge green">Yes ('+(p.invite_count||0)+')</span>':'<span class="badge gray">No</span>';var st=p.is_logged_in?'<span class="badge green">Active</span>':'<span class="badge amber">Pending</span>';var inviteBtnLabel=p.invited_at?'Resend Invite':'Invite';h+='<tr><td>'+(p.phone||'')+'</td><td>'+(p.name||'Parent')+'</td><td>'+(p.school_name||'-')+'</td><td>'+(p.kid_count||0)+(p.kid_names&&p.kid_names.length?' <span style="font-size:11px;color:var(--ink-faint)">'+p.kid_names.join(', ')+'</span>':'')+'</td><td>'+inv+'</td><td>'+st+'</td><td style="white-space:nowrap;display:flex;gap:6px"><button class="btn-orange" onclick="getInviteLink(&#39;'+(p.phone||'')+'&#39;,&#39;sms&#39;)">'+inviteBtnLabel+'</button><button class="btn-sm" onclick="getInviteLink(&#39;'+(p.phone||'')+'&#39;,&#39;copy&#39;)">Copy Link</button></td></tr>'}h+='</tbody></table>';document.getElementById('parentsTable').innerHTML=h}).catch(function(e){document.getElementById('parentsTable').innerHTML='<p style="color:var(--alert);padding:12px;font-weight:500;font-size:12px">Error: '+e.message+'</p>'})}
function loadStudents(){var s=document.getElementById('studentSearch').value.trim();var qs=s?('?q='+encodeURIComponent(s)):'';document.getElementById('studentsTable').innerHTML='<div class="loading"><div class="spinner"></div></div>';api('GET','/api/admin/students'+qs).then(function(rows){if(!rows||rows.length===0){document.getElementById('studentsTable').innerHTML='<div class="empty-state"><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21.42 10.922a1 1 0 0 0-.019-1.838L12.83 5.18a2 2 0 0 0-1.66 0L2.6 9.08a1 1 0 0 0 0 1.832l8.57 3.908a2 2 0 0 0 1.66 0z"/><path d="M22 10v6"/><path d="M6 12.5V16a6 3 0 0 0 12 0v-3.5"/></svg><p>No students found. Imported students appear here with their parent\\'s details.</p><div style="margin-top:12px"><button class="btn-sm" onclick="switchTab(&#39;import&#39;)">Go to Import</button></div></div>';return}var h='<table><thead><tr><th>Student</th><th>Student ID</th><th>Age</th><th>Gender</th><th>Grade</th><th>Height / Weight</th><th>School</th><th>Camps</th><th>Parent</th><th>Parent Phone</th><th>Dental / Eye / Nutrition</th><th>Last Checkup</th><th>Parent Login</th></tr></thead><tbody>';for(var i=0;i<rows.length;i++){var k=rows[i];var login=k.parent_logged_in?'<span class="badge green">Active</span>':'<span class="badge amber">Pending</span>';var hw=(k.height_cm||0)+' cm / '+(k.weight_kg||0)+' kg';var health=(k.dental||k.eyesight||k.nutrition)?[k.dental||'-',k.eyesight||'-',k.nutrition||'-'].join(' / '):'<span style="color:var(--ink-faint)">Not checked</span>';var campHtml=(k.camps||[]).length?k.camps.map(function(c){return '<b>'+(c.title||'Camp')+'</b>'+(c.date?' <span style="font-size:11px;color:var(--ink-faint)">('+c.date+')</span>':'')}).join(' &nbsp;·&nbsp; '):'-';h+='<tr><td><b>'+(k.name||'-')+'</b></td><td>'+(k.student_id||(k.student_ref||'').replace(/^stu_/,'')||'-')+'</td><td>'+(k.age!=null?k.age:'-')+'</td><td>'+(k.gender||'-')+'</td><td>'+(k.grade||'-')+'</td><td>'+hw+'</td><td>'+(k.school||'-')+'</td><td>'+campHtml+'</td><td>'+(k.parent_name||'-')+'</td><td>'+(k.parent_phone||'-')+'</td><td>'+health+'</td><td>'+(k.last_checkup||'Not yet')+'</td><td>'+login+'</td></tr>'}h+='</tbody></table>';document.getElementById('studentsTable').innerHTML=h}).catch(function(e){document.getElementById('studentsTable').innerHTML='<p style="color:var(--alert);padding:12px;font-weight:500;font-size:12px">Error: '+e.message+'</p>'})}
function addParentStudent(){var parentName=document.getElementById('apsParentName').value.trim();var phone=document.getElementById('apsPhone').value.trim();var studentName=document.getElementById('apsStudentName').value.trim();var schoolId=document.getElementById('apsSchoolSelect').value;var age=parseInt(document.getElementById('apsAge').value,10)||0;var gender=document.getElementById('apsGender').value;var grade=document.getElementById('apsGrade').value.trim();var studentId=document.getElementById('apsStudentId').value.trim();var campId=document.getElementById('apsCampSelect')?document.getElementById('apsCampSelect').value:'';var heightCm=parseFloat(document.getElementById('apsHeight').value)||0;var weightKg=parseFloat(document.getElementById('apsWeight').value)||0;if(!parentName||!phone||!studentName||!schoolId){document.getElementById('apsResult').innerHTML='<div class="banner warn" style="display:flex">Please fill Parent Name, Parent Phone, Student Name and select a School.</div>';return}
var row={phone:phone,parentName:parentName,studentName:studentName,schoolId:schoolId,dental:'GOOD',eyesight:'GOOD',nutrition:'GOOD'};if(age>0)row.age=age;if(gender)row.gender=gender;if(grade)row.grade=grade;if(studentId)row.studentId=studentId;if(campId)row.campId=campId;if(heightCm>0)row.heightCm=heightCm;if(weightKg>0)row.weightKg=weightKg;
var btn=document.getElementById('apsSaveBtn');btn.disabled=true;btn.textContent='Adding...';
api('POST','/api/admin/import',{rows:[row],dryRun:false,generateLinks:true,resendExisting:false,filename:'manual_entry'}).then(function(r){btn.disabled=false;btn.textContent='Add Parent & Student';var row2=r&&r.results&&r.results[0]?r.results[0]:null;var ok=row2&&(row2.status==='created'||row2.status==='updated');var h=ok?'<div class="banner" style="display:flex;gap:8px;align-items:center;background:rgba(16,185,129,.1);color:var(--good);border:none"><span>&#10003;</span> <b>'+(row2.status==='created'?'Added':'Updated')+'</b> '+studentName+' &mdash; parent is ready to be invited.</div>':'<div class="banner warn" style="display:flex">'+((row2&&row2.message)||'Could not add. Check the details and try again.')+'</div>';
if(row2&&row2.link){lastImportLinks.push({phone:phone,link:row2.link});h+='<div style="margin-top:10px"><label>Invite Link</label><div style="display:flex;gap:8px"><input type="text" id="apsLinkBox" value="'+row2.link+'" readonly style="flex:1"><button class="btn-primary" onclick="copyText(document.getElementById(&#39;apsLinkBox&#39;).value,this)">Copy Link</button></div><p class="helper-text">Share this link with the parent (WhatsApp, SMS, email) &mdash; it opens sign-in with their number pre-filled.</p></div>'}
if(ok)h+='<div style="margin-top:8px"><button class="btn-sm" onclick="switchTab(&#39;parents&#39;)">Invite the parent &rarr;</button></div>';document.getElementById('apsResult').innerHTML=h;if(ok)document.getElementById('apsForm').reset();loadStudents();loadOverview()}).catch(function(e){btn.disabled=false;btn.textContent='Add Parent & Student';document.getElementById('apsResult').innerHTML='<div class="banner warn" style="display:flex">Error: '+e.message+'</div>'})}
function getInviteLink(phone,mode){if(!phone)return;phone=String(phone);openModal({title:mode==='sms'?'Sending invite...':'Preparing link...',message:'Please wait.',confirmText:false,cancelText:false,icon:'<div class=\"spinner\"></div>'});api('POST','/api/admin/invite',{phones:[phone]}).then(function(d){
  var detail=d.details&&d.details[0];
  if(!detail||detail.status!=='linked'||!detail.link){
    var reason=detail&&detail.reason?detail.reason:'Could not generate a link for this number.';
    openModal({title:'Could Not Generate Link',message:reason,confirmText:'OK',cancelText:false,iconBg:'rgba(239,68,68,.12)',iconColor:'var(--alert)',icon:'<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"26\" height=\"26\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><circle cx=\"12\" cy=\"12\" r=\"10\"/><path d=\"M15 9l-6 6\"/><path d=\"m9 9 6 6\"/></svg>'});return
  }
  ...
  if(mode==='share'&&navigator.share){
    closeModal(false);
    navigator.share({title:'VitaHero',text:'Your child\\'s health report is ready on VitaHero. Install: '+link,url:link}).catch(function(){});
    return
  }
  navigator.clipboard.writeText(link).then(function(){
    openModal({title:'Link Copied',message:'Invite link for '+phone+' has been copied to your clipboard:\\n\\n'+link,confirmText:'OK',cancelText:false,iconBg:'rgba(16,185,129,.12)',iconColor:'var(--good)',icon:'<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"26\" height=\"26\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2.5\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"M20 6 9 17l-5-5\"/></svg>',onClose:function(){loadParents()}})
  }).catch(function(){
    openModal({title:'Invite Link',message:link,confirmText:'OK',cancelText:false,onClose:function(){loadParents()}})
  })
}).catch(function(e){openModal({title:'Link Failed',message:'Failed to generate invite link: '+e.message,confirmText:'OK',cancelText:false,iconBg:'rgba(239,68,68,.12)',iconColor:'var(--alert)',icon:'<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"26\" height=\"26\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><circle cx=\"12\" cy=\"12\" r=\"10\"/><path d=\"M15 9l-6 6\"/><path d=\"m9 9 6 6\"/></svg>'})})}
...
function loadDoctorsTab(){loadCampOptions();loadHospitalsForDoctorRows();loadDoctors()}
function hospitalOptionsHtml(selectedId){var h='<option value="">No hospital linked yet</option>';for(var i=0;i<hospitalsCache.length;i++){var hosp=hospitalsCache[i];var sel=selectedId&&selectedId===hosp.id?' selected':'';h+='<option value="'+hosp.id+'"'+sel+'>'+(hosp.name||'Hospital')+(hosp.city?' - '+hosp.city:'')+'</option>'}return h}
function toggleForm(id,btn,openLabel){var el=document.getElementById(id);if(!el)return;var willShow=(el.style.display==='none');el.style.display=willShow?'':'none';if(btn)btn.textContent=willShow?'Close':(openLabel||'Open');if(willShow){var f=el.querySelector('input,select');if(f)f.focus();el.scrollIntoView({behavior:'smooth',block:'start'})}}
function loadHospitalsForDoctorRows(){api('GET','/api/admin/hospitals').then(function(rows){hospitalsCache=rows||[];var selects=document.querySelectorAll('#doctorRowsContainer .doc-hospital');for(var i=0;i<selects.length;i++){var cur=selects[i].value;selects[i].innerHTML=hospitalOptionsHtml(cur)}}).catch(function(){})}
function loadHospitals(){document.getElementById('hospitalsTable').innerHTML='<div class="loading"><div class="spinner"></div></div>';api('GET','/api/admin/hospitals').then(function(rows){hospitalsCache=rows||[];var selects=document.querySelectorAll('#doctorRowsContainer .doc-hospital');for(var i=0;i<selects.length;i++){var cur=selects[i].value;selects[i].innerHTML=hospitalOptionsHtml(cur)}if(!rows||rows.length===0){document.getElementById('hospitalsTable').innerHTML='<div class="empty-state"><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 6v4"/><path d="M14 14h-4"/><path d="M14 18h-4"/><path d="M14 8h-4"/><path d="M18 12h2a2 2 0 0 1 2 2v6a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2v-6a2 2 0 0 1 2-2h2"/><path d="M6 12V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v8"/></svg><p>No hospitals added yet. Click Add Hospital above so parents can find where your doctors practice.</p></div>';return}var h='<table><thead><tr><th>Name</th><th>Address</th><th>City / Area</th><th>Pincode</th><th>Camp Partner</th><th>Actions</th></tr></thead><tbody>';for(var i=0;i<rows.length;i++){var hh=rows[i];var partner=hh.is_camp_partner?'<span class="badge orange">Yes</span>':'<span class="badge gray">No</span>';h+='<tr><td><b>'+(hh.name||'')+'</b></td><td>'+(hh.address||'<span style="color:var(--alert)">Missing</span>')+'</td><td>'+(hh.city||'-')+(hh.district?' / '+hh.district:'')+'</td><td>'+(hh.pincode||'-')+'</td><td>'+partner+'</td><td><button class="btn-sm" onclick="editHospital('+i+')">Edit</button></td></tr>'}h+='</tbody></table>';document.getElementById('hospitalsTable').innerHTML=h}).catch(function(e){document.getElementById('hospitalsTable').innerHTML='<p style="color:var(--alert);padding:12px;font-weight:500;font-size:12px">Error: '+e.message+'</p>'})}
function saveHospital(){var name=document.getElementById('hospName').value.trim();var address=document.getElementById('hospAddress').value.trim();var city=document.getElementById('hospCity').value.trim();var district=document.getElementById('hospDistrict').value.trim();var phone=document.getElementById('hospPhone').value.trim();var pincode=document.getElementById('hospPincode').value.trim();var campPartner=document.getElementById('hospCampPartner').checked;if(!name||!address||!city){document.getElementById('hospResult').innerHTML='<div class="banner warn" style="display:flex">Please fill Hospital Name, Address, and City.</div>';return}var btn=document.getElementById('hospSaveBtn');var editId=btn.getAttribute('data-edit-id');btn.disabled=true;btn.textContent=editId?'Updating...':'Saving...';var payload={name:name,address:address,city:city,district:district,phone:phone,pincode:pincode,is_camp_partner:campPartner};var method=editId?'PUT':'POST';var url=editId?('/api/admin/hospitals/'+editId):'/api/admin/hospitals';api(method,url,payload).then(function(r){btn.disabled=false;btn.textContent='Save Hospital';btn.removeAttribute('data-edit-id');document.getElementById('hospCancelBtn').style.display='none';document.getElementById('hospFormHint').textContent='Add the hospitals your doctors work at, with a full address, so parents can find the nearest one to book an appointment. Fields marked with * are required.';document.getElementById('hospResult').innerHTML='<div class="banner info" style="display:flex"><span>&#10003;</span> Hospital '+(editId?'updated':'added')+': <b>'+(r.name||name)+'</b></div>';resetHospitalForm();loadHospitals()}).catch(function(e){btn.disabled=false;btn.textContent=editId?'Update Hospital':'Save Hospital';document.getElementById('hospResult').innerHTML='<div class="banner warn" style="display:flex">Failed: '+e.message+'</div>'})}
function editHospital(idx){var hh=hospitalsCache[idx];if(!hh)return;var fc=document.getElementById('hospFormCard');if(fc)fc.style.display='';document.getElementById('hospName').value=hh.name||'';document.getElementById('hospAddress').value=hh.address||'';document.getElementById('hospCity').value=hh.city||'';document.getElementById('hospDistrict').value=hh.district||'';document.getElementById('hospPhone').value=hh.phone||'';document.getElementById('hospPincode').value=hh.pincode||'';document.getElementById('hospCampPartner').checked=!!hh.is_camp_partner;var btn=document.getElementById('hospSaveBtn');btn.textContent='Update Hospital';btn.setAttribute('data-edit-id',hh.id);document.getElementById('hospCancelBtn').style.display='inline-flex';document.getElementById('hospFormHint').textContent='Editing hospital: '+(hh.name||'')+'. Make changes and click Update Hospital.';document.getElementById('hospResult').innerHTML='';window.scrollTo({top:0,behavior:'smooth'})}
function cancelHospitalEdit(){resetHospitalForm();var btn=document.getElementById('hospSaveBtn');btn.removeAttribute('data-edit-id');btn.textContent='Save Hospital';document.getElementById('hospCancelBtn').style.display='none';document.getElementById('hospFormHint').textContent='Add the hospitals your doctors work at, with a full address, so parents can find the nearest one to book an appointment. Fields marked with * are required.';document.getElementById('hospResult').innerHTML=''}
function resetHospitalForm(){document.getElementById('hospName').value='';document.getElementById('hospAddress').value='';document.getElementById('hospCity').value='';document.getElementById('hospDistrict').value='';document.getElementById('hospPhone').value='';document.getElementById('hospPincode').value='';document.getElementById('hospCampPartner').checked=false}
...
function loadApsOptions(){api('GET','/api/admin/schools').then(function(schools){var sel=document.getElementById('apsSchoolSelect');if(!sel)return;if(!schools||schools.length===0){sel.innerHTML='<option value="">No schools yet - they appear here once created</option>';return}var h='<option value="">Select a school...</option>';for(var i=0;i<schools.length;i++){var s=schools[i];h+='<option value="'+s.id+'">'+(s.name||'School')+(s.city?' - '+s.city:'')+'</option>'}sel.innerHTML=h}).catch(function(){var sel=document.getElementById('apsSchoolSelect');if(sel)sel.innerHTML='<option value="">Failed to load schools</option>'});api('GET','/api/admin/camps').then(function(camps){var sel=document.getElementById('apsCampSelect');if(!sel)return;if(!camps||camps.length===0){sel.innerHTML='<option value="">No camps yet - create one in the Camps tab</option>';return}var h='<option value="">No camp registration</option>';for(var i=0;i<camps.length;i++){var c=camps[i];h+='<option value="'+c.id+'">'+(c.title||'Camp')+(c.school_name?' - '+c.school_name:'')+(c.date?' ('+c.date+')':'')+'</option>'}sel.innerHTML=h}).catch(function(){var sel=document.getElementById('apsCampSelect');if(sel)sel.innerHTML='<option value="">Failed to load camps</option>'})}
function addDoctorRow(){docRowCounter++;var container=document.getElementById('doctorRowsContainer');var div=document.createElement('div');div.className='doc-row';div.setAttribute('data-row',docRowCounter);div.innerHTML='<div class="form-group"><input type="text" class="doc-name" placeholder="Dr. ..."></div><div class="form-group"><input type="text" class="doc-phone" placeholder="9876543210"></div><div class="form-group"><select class="doc-specialty"><option value="Dental">Dentist (Dental Check)</option><option value="Ophthalmology">Ophthalmologist (Vision Screening)</option><option value="ENT">ENT Specialist (Hearing & General)</option><option value="Nutrition">Nutritionist (Nutrition & Anaemia)</option><option value="Paediatrics">Paediatrician (Full Checkup)</option><option value="General Paediatrics" selected>General Paediatrics (Full Checkup)</option><option value="Dermatology">Dermatologist (Skin & General)</option></select></div><div class="form-group"><select class="doc-hospital">'+hospitalOptionsHtml()+'</select></div><button class="btn-remove" onclick="removeDoctorRow(this)"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg></button>';container.appendChild(div);updateRemoveButtons();updateDocCount()}
