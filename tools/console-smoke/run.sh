...
status=0
for f in "$here"/screens.mjs "$here"/photo-gating.mjs "$here"/consent-loop.mjs "$here"/phone.mjs "$here"/dashboard.mjs "$here"/oversight.mjs "$here"/doctor-camp.mjs "$here"/navigation.mjs "$here"/actions.mjs "$here"/school-lifecycle.mjs "$here"/manage.mjs "$here"/admin-panel.mjs "$here"/sign-in.mjs "$here"/first-run.mjs "$here"/state.mjs; do
  echo "── $(basename "$f")"
